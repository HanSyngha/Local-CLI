...
/**
 * Get permanent profile directory path
 */
export function getProfileDir(): string {
  const platform = getPlatform();
  if (platform === 'wsl') {
    // WSL: Save profile to Windows side (Chrome runs on Windows)
    // Bypass sh with spawnSync → $env:LOCALAPPDATA is directly interpreted in PowerShell
    try {
